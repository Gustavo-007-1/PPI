            ...
            if($qtd_turma>0){
                while($row_turma = $res_turma->fetch_object()){
                    if($row_turma->id_curso == $row_curso->id_curso){
                        ...
                        if($_SESSION["categoria"]==1 || $_SESSION["categoria"]==2){
                            echo "<button onclick=\"location.href='?page=editar_turma&id=" . $row_turma->id_turma . "';\">Editar</button>";
                            echo " <button onclick=\"if(confirm('Tem certeza que deseja excluir?')){location.href='?page=salvar_turma&acao=excluir&id=".$row_turma->id_turma."';}else{false;}\"'>Excluir</button>";
                        }
                    }
                }
